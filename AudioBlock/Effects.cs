...
namespace WDE.AudioBlock
{
    public static class Effects
    {
        ...
        public static void Resample(ref float[] sourceData, ref float[] destData, double outputRate, double inputRate)
        {
            ...
            var r8bR = new R8brain();
            r8bR.Create(inputRate, outputRate, BUFFER_LENGTH, 2.0, ER8BResamplerRes.r8brr24);
            ...
            do
            {
                ...
                double[] inputBufferL = new double[inputBufferFillLevel / 2];
                for (int i = 0; i < inputBufferFillLevel / 2; i++)
                {
                    inputBufferL[i] = sourceData[totalInputSampleCount + 2 * i];
                }

                outputSampleCount = r8bL.Process(inputBufferL, inputBufferFillLevel / 2, out outputBuffer);

                for (int i = 0; i < outputSampleCount; i++)
                {
                    destData[totalOutputSampleCount + 2 * i] = (float)outputBuffer[i];
                }

                double[] inputBufferR = new double[inputBufferFillLevel / 2];
                for (int i = 0; i < inputBufferFillLevel / 2; i++)
                {
                    inputBufferR[i] = sourceData[totalInputSampleCount + 2 * i + 1];
                }

                outputSampleCount = r8bR.Process(inputBufferR, inputBufferFillLevel / 2, out outputBuffer);

                for (int i = 0; i < outputSampleCount; i++)
                {
                    destData[totalOutputSampleCount + 2 * i + 1] = (float)outputBuffer[i];
                }
                ...
                totalOutputSampleCount += outputSampleCount * 2;
            }
